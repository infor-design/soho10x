  ...
  $.fn.multiselect = function(options, args) {
    ...
    Plugin.prototype = {
      ...
      openList: function(term) {
        ...
        self.list.parent().css('height','');
        self.list.css('height','');
        var h = self.list.height();
        self.list.parent().height(h);
        self.list.height(h);
        self.input.focus();
      },
      ...
      addTag: function(tag) {
        ...
        sel[0].selected = true;
        tagSpan = $('<span class="tag">' + tag.text() + '<span class="remove"></span></span>');
