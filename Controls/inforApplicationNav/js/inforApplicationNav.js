    ...
    _handleKeys: function (e, elem) {
      var self = this,
        ...
        isTop = focusd.parent().parent().parent().hasClass("inforApplicationNav"),
                closestTopLi = li.closest('.inforApplicationNav > ul > li'),
                isSubmenu = closestTopLi.children("ul").get(0) !== li.parent().get(0),
        next, prev;
      ...
      switch (e.keyCode) {
        ...
        case 27: // escape
            if (isTop) {
                ...
                li.parent().prev('a').focus().parent('li').removeClass('activeHeader');
                li.parent().hide().attr({
                    'aria-hidden': 'true',
                    'aria-expanded': 'false'
                });
        ...
        case 37: //left
          if (isTop) {
            ...
            if (isSubmenu) {
              ...
              li.parent().hide().attr({
                ...
                //focus the previous top level li
                prev = closestTopLi.prevAll(':visible:first');
                if (prev.length === 0) {
                    prev = closestTopLi.parent().find("li:visible:last");
                }
                prev.children("a").focus();
                self.closeOpenMenus(elem);
            }
          ...
          return false;
        case 38: //up
          if (isTop && li.children('ul').length !== 0) {
            self.openMenu(li.find("ul:first"), li);
          }
          else {
            prev = li.prev();
            ...
            else if (prev.is(':not(:visible)')){
              self._moveUp(li.parent());
            }
            prev.find("a").focus();
          }
          e.stopPropagation();
          ...
          if (isTop) {
            ...
            if (li.find("ul:first").length > 0) {
              self.openMenu(li.find("ul:first"), li);
                                ...
                                next = closestTopLi.next(':visible');
                                if (next.length === 0) {
                                    next = closestTopLi.parent().find('li:visible:first');
                                }
                                next.children('a').focus();
                                self.closeOpenMenus(elem);
                            }
        ...
        case 40: //down
          if (isTop && li.children('ul').length !== 0) {
            ...
            else if (next.is(':not(:visible)')){
                self._moveDown(li.parent());
            }
            next.find("a").focus();
          }
          e.stopPropagation();
    ...
    buildmenu: function (elementId) {
      ...
      $mainmenu.find("a").hover(function () {
        ...
        $(this).removeClass("hover");
            ...
            $("#" + elementId).attr('role', 'navigation');
            $mainmenu.attr('role', 'menubar');
            $mainmenu.find("li:not(:first) > a").attr("tabindex", "-1");
            $mainmenu.find('a').first().attr('tabindex', '0');
            this._applyARIA($mainmenu);
      this._attachSubMenus($headers);
      this._setupSessionNav();
    },
        _applyARIA: function ($ul) {
            ...
            $ul.find('a').attr('role', 'menuitem');
            $ul.find('li > ul').each(function(index) {
                var $ul = $(this),
                    $a = $ul.prev('a'),
                    aId = $a.attr('id') || false;

                if (aId === false) {
                    aId = 'appNavA' + time + index;
                    $a.attr('id', aId);
                }

                $a.attr('aria-haspopup', 'true');
                $ul.attr('aria-labelledby', aId);
            });
        },
...
//      submenu.width(submenu.outerWidth()); //width keeps growing because of this 
      ...
      //Save to each menu
      this._saveDataToMenu(submenu, up, down);
    },
    _moveUp: function (submenu) {
      //Scroll up one element
      var hiddenElms = this._getHiddenElems(submenu),
      visibleSubMenus = submenu.find('li:visible').not('.scrollUp, .scrollDown'),
      upButton = $.data(submenu[0], "upButton"),
      downButton = $.data(submenu[0], "downButton"),
      topElem = null,
      bottomElem = null;
      ...
      topElem = hiddenElms[0][hiddenElms[0].length-1];
      if (topElem) {
        topElem.show();
      }

      //remove last item from the visible items and hide it.
      bottomElem = visibleSubMenus.last();
      bottomElem.hide();

      //disable if there are no more.
      this._enableVerticalScrollBars(submenu);
      this._saveDataToMenu(submenu, upButton, downButton);
    },

    _getHiddenElems: function (submenu) {
      var hiddenTopItems = [],
        hiddenBottomItems = [];

       var firstVisible = submenu.children('li:visible').not('.scrollUp, .scrollDown').first().index(),
        lastVisible = submenu.children('li:visible').not('.scrollUp, .scrollDown').last().index();

      submenu.children('li').each(function () {
        var li = $(this);

        if (li.is(':hidden') && li.index() < firstVisible) {
          hiddenTopItems.push(li);
        }

        if (li.is(':hidden') && li.index() > lastVisible) {
          hiddenBottomItems.push(li);
        }

      });

      return [hiddenTopItems, hiddenBottomItems];
    },
    ...
    _moveDown: function (submenu) {
      ...
      var hiddenElms = this._getHiddenElems(submenu),
      visibleSubMenus = submenu.children('li:visible').not('.scrollUp, .scrollDown'),
      upButton = $.data(submenu[0], 'upButton'),
      downButton = $.data(submenu[0], 'downButton'),
      bottomElem = null,
      topElem = null;
      ...
      bottomElem = hiddenElms[1][0];
      bottomElem.show();

      //remove an item from top - hide it
      visibleSubMenus.first().hide();
    ...
    _enableVerticalScrollBars: function (submenu) {
      var up = $.data(submenu[0], "upButton"),
      down = $.data(submenu[0], "downButton"),
      hiddenElms = this._getHiddenElems(submenu);
    ...
    _saveDataToMenu: function (submenu, upButton, downButton) {
       $.data(submenu[0], "upButton", upButton);
      $.data(submenu[0], "downButton", downButton);
    },
    ...
    _setOverflow: function () {
      ...
      var self = this,
        ...
        expectedOffset = Math.round(rootDiv.position().top) + 2,
        itemCount = 1,
    ...
    closeOpenMenus: function (root) {
      ...
      root.find('ul li ul').filter(":visible").not(".inforMegaMenuColumn").hide().attr({
                'aria-hidden': 'true',
                'aria-expanded': 'false'
            });
    ...
    _closeSiblingMenus: function ($curobj) {     //Close ALL Menus at the same level (no animation)
      ...
      $curobj.siblings("li").removeClass("activeHeader").children("ul").filter(":visible").not(".inforMegaMenuColumn").each(function () {
        ...
        $targetul.children("ul").filter(":visible").not(".inforMegaMenuColumn").hide().attr({
                    'aria-hidden': 'true',
                    'aria-expanded': 'false'
                });
    ...
    openMenu: function ($subul, $curobj, isCallback) {
      ...
      if ($subul.queue().length <= 1) {
        if (!Globalize.culture().isRTL) {
          $subul.css({
            left: menuleft + ($subul.parent('li').index() === 0 ? 28 : 12) + "px"
          }).animate({
            height: 'show',
            opacity: 'show'
          }, 100, function () {
      ...
      //this is the expected height of the menu numItems*33 (the width of each li)
      expectedHeight = ($subul.children("li:visible").length * 33);
...
//
      ...
      $subul.parent("li").addClass("activeHeader");
      rootDiv.find("a.hover").removeClass("hover");
      $subul.find(".inforMegaMenuColumn").show();
      //if the submenu was too big to fit on the screen, make sure it's all the way at the top
      if ($subul.find('li:first').hasClass('scrollUp')) {
        numToScroll = $subul.children('li:not(.scrollUp):visible:first').index() - 1; //subtract 1 from index because of .scrollUp li
        for (var i = 0; i < numToScroll; i++) {
          self._moveUp($subul);
        }
      }
      $subul.find('li a').first().focus();
      $subul.attr({
          'aria-hidden': 'false',
          'aria-expanded': 'true'
      });
