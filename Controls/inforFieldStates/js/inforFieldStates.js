	...
	*/
	$.fn.setValue = function (value) {
		return this.each(function () {
			...
			//Drop Downs.
			if ($.nodeName(elem, "select") || $elem.hasClass("inforDataGridDropDownList")) {
				if (value == undefined || value === "") {
					$elem.prop("selectedIndex", -1);
					$elem.next().find('.selectedSingle span').text("");
					return null;
				}
				...
				//BROKEN if (api && api.options.displayCodeOnly) {
				$elem.val(value);

				//set the input value
				var input = $elem.next().find('.selectedSingle span');
				input.text(value);
				return value;
			}
			...
			if (elem.hasClass("inforCheckbox"))
			{
				...
				if (isAdd && hasLabel)
				{
					required.css((!Globalize.culture().isRTL)
						? {"top": "-6px", "left": "9px", "margin-left": "-9px"}
	...
	$.fn.disable = function (options) {
		...
		return this.each(function () {
			...
			if ($control.hasClass("inforDropDownList")) {
				$control.attr(targetClass, targetClass).trigger("updated");
				return;
			}
	...
	*/
	$.fn.enable = function () {
		...
		return this.each(function () {
			...
			if ($control.hasClass("inforDropDownList")) {
				$control.removeAttr(targetClass, targetClass).trigger("updated");
				return;
			}
	...
	$.fn.trackDirty = function (settings) {
		...
		changeCallback = function () {
			...
			if (isChangedBack) {
				...
				if ($el.is("select")) {
					$el.next().find("input").removeClass("isDirty");
				}
				return;
			}
			...
			if ($el.is("select")) {
				$el.next().find("input").addClass("isDirty");
			}
